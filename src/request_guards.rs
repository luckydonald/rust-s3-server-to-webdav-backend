...
impl<'r> FromRequest<'r> for AuthCheckPassed {
    ...
    async fn from_request(request: &'r rocket::Request<'_>) ->
        ...
        if config.is_err() {
            return rocket::request::Outcome::Failure((Status::InternalServerError, AuthError::ServerMisconfiguration));
        }
        let env = config.unwrap();
        if api_key.is_none() {
        ...
        if is_valid {
            ...
            rocket::request::Outcome::Failure((Status::Forbidden, AuthError::InvalidSignature))
        }
    }
}
